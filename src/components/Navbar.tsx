...
const DisabledNavItem: React.FC<{ link: NavigationLink; isMobile?: boolean }> = ({ link, isMobile }) => (
  <span
    className={`relative block px-4 py-2 text-sm font-medium text-text-tertiary ${
      isMobile ? 'rounded-md' : ''
    } cursor-not-allowed`}
    aria-disabled="true"
  >
  ...
  const navLinks: NavigationLink[] = [
    ...
    { to: '/about', label: 'About' },
    {
      ...
      children: [
        ...
        { to: '/leadership/special-recognition', label: 'Special Recognition' },
      ],
    },
    { to: '/projects', label: 'Projects' },
    { to: '/achievements', label: 'Achievements' },
    { to: '/quiz', label: 'Quiz', status: 'Coming Soon', disabled: true },
    { to: '/leaderboard', label: 'Leaderboard', status: 'Coming Soon', disabled: true },
  ];
        ...
        <div className="md:hidden bg-sand-100 border-t border-sand-300 shadow-md">
          <ul className="flex flex-col px-4 py-3 space-y-1">
            {navLinks.map((link) => (
              <li key={link.to}>
                {link.disabled ? (
                  <DisabledNavItem link={link} isMobile={true} />
                ) : (
