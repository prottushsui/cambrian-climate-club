...
const Navbar: React.FC = memo(() => {
  ...
  return (
    <nav
      className="bg-sandstone-100/95 backdrop-blur border-b border-sandstone-200 fixed w-full z-50 top-0 start-0 transition-all duration-300"
    ...
    >
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto px-4 py-3">
        ...
        >
          ...
          <span className="self-center text-lg font-semibold tracking-wide text-charcoal-900 hidden sm:block">
        ...
        <div className="hidden md:block w-full md:w-auto" id="navbar-default">
          <ul className="font-medium flex flex-row space-x-2 p-0 mt-0 border-0 bg-transparent">
            {navLinks.map(link => (
              <li key={link.to}>
                <NavItem to={link.to} onClick={closeMenu}>
                  {link.label}
                </NavItem>
              </li>
            ))}
            ...
            <ul className="flex flex-col p-4 font-medium">
              {navLinks.map(link => (
                <li key={link.to} className="mb-1 last:mb-0">
                  <NavItem to={link.to} onClick={closeMenu} isMobile={true}>
                    {link.label}
                  </NavItem>
                </li>
              ))}
