...
const Carousel: React.FC<{
  ...
  title: string;
}> = memo(({ mediaItems, onMediaClick, title }) => {
...
const ImageGallery: React.FC<ImageGalleryProps> = memo(({ galleries }) => {
  ...
  const allGalleries: Gallery[] = [
    { title: 'Video Archive', images: VIDEO_ARCHIVE },
    ...galleries,
  ];
  ...
  return (
    <div className="container mx-auto px-4 py-16">
      <SectionHeader
        title="Image and Video Archive"
        subtitle="Explore the memorable moments from our events and initiatives."
      />
      <div className="space-y-20">
        {allGalleries.map((gallery, index) => (
          ...
          >
            ...
            <Carousel
              mediaItems={convertToMediaItems(gallery)}
      ...
      {videoModalUrl && (
        <VideoModal videoUrl={videoModalUrl} onClose={closeVideoModal} />
      )}
    </div>
  );
});
